...
package ui.web.org.springbyexample.jdbc.core;
        ...
        if (lSqlScripts != null) {
            for (String sqlScript : lSqlScripts) {
                ...
                if (StringUtils.isNotEmpty(sql)) {
                    ...
                    String delimiter = ";";
                    Scanner scanner = new Scanner(sql).useDelimiter(delimiter);

                    while(scanner.hasNext()) {

                        // Get statement
                        String rawStatement = scanner.next() + delimiter;
                        System.out.println("execute now "+rawStatement);
                        // execute sql
                        if(StringUtils.isNotEmpty(rawStatement)  && rawStatement.length()>5){
                            //template.execute(rawStatement);
                        }
